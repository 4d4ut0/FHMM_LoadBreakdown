...
def create_combined_hmm(model):
    ...
    print("MODEL COMBINED---------------------> ", combined_model)
    print("combined_model.startprob", pi_combined)
    print("combined_model.means_", mean_combined)
...
def decode_hmm(length_sequence, centroids, appliance_list, states):
    ...
    for i in range(length_sequence):
        ...
        for appliance in appliance_list:
            ...
            print("len(centroids[appliance]>>>> ", len(centroids[appliance]))
            factor = factor // len(centroids[appliance])
            print("FACTOR2>>>> ", factor)
            print("int(states[i]) >>>> ", int(states[i]))
            ...
            print("hmm_states[appliance][i] >>>> ", hmm_states[appliance][i])
...
class FHMM(Disaggregator):
    ...
    def train(self, metergroup, num_states_dict={}, **load_kwargs):
        ...
        #print("learnt_model",learnt_model)
        num_meters = len(metergroup.meters)
        #print("num_meters", num_meters)
        if num_meters > 12:
            max_num_clusters = 2
        else:
            max_num_clusters = 3
        ...
        for i, meter in enumerate(metergroup.submeters().meters):
            ...
            print("power_series>>>>>>>>>>", power_series)
            ...
            print("learnt_model[meter]>>>>>>>>>>>>>>>>>>>>>>", learnt_model[meter])

            # Fit
            print("ESSE EH O X>>>>>>> ", X)
            learnt_model[meter].fit(X)
            print("learnt_model2 >>>>>>> ", learnt_model[meter])
...
#Essa função é a desagregação:::
        for learnt_states in learnt_states_array:
            ...
            print("means>>>>>>", means)
            print("means.keys()>>>>>>", means.keys())
            print("learnt_states>>>>>>", learnt_states)

            print("decoded_power>>>>>>", decoded_power)
            print("decoded_states>>>>>>", decoded_states)

            decoded_states_array.append(decoded_states)
            decoded_power_array.append(decoded_power)
            print("decoded_power_array[0]>>>>>", decoded_power_array[0])

        prediction = pd.DataFrame(
            decoded_power_array[0], index=test_mains.index)
        print("PREDICTION>>>>", prediction)
        #np.savetxt("prediction1.csv", prediction, delimiter=",")
