...
boolean nexReconnect() 
{
  ...
  if (!WiFi.isConnected())
  {
    ...
    WiFi.mode(WIFI_STA);
    WiFi.begin(ssid, password);
    nexDelay(500); //tempo maximo para att a variavel
    if ( WiFi.status() != WL_CONNECTED )
    {
        if(countReset >= FAILED_ATTEMPT){
          storeDataEnergy();
          ESP.restart();
        }
...
void nexDelay(long milliseconds)
{
    long d;
    d = millis();

    while (millis()-d < milliseconds) {
        yield(); //deixa o esp trabalhar durante o delay
    }
}
////////////////////////////////FIM////////////////////////////////////////
